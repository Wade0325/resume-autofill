...
匯入方向相反（已填履歷 → 我的資料）：模型連同**頁面截圖**一起判讀，排版資訊
補回攤平文字丟失的部分；104 的 PDF 由 pypdfium2 直接取圖，Word 履歷才需要
LibreOffice 轉一手。PDF 取字一定要 NFKC——104 的字型把中文對映到康熙部首區，
不正規化的話逐字驗證會把每個值都判成幻覺。
