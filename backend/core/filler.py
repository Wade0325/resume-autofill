...
class Pages:
    ...
    一列一筆、補漏兩輪只附這一批位置所在的那幾張（`for_addrs`）——那幾輪要的是
    「找得到位置」。配對那一輪接在同一串對話上問（`llm.Chat`，不必換掉結尾，快取整段
    都吃得到），只附這一批位置所在、而且這串對話裡還沒附過的那幾張，多數批次一張都
    不用附。勾選題整份附上：那一輪要判斷「這題在問哪一項資料」，少看一張會改判
    （連跑兩輪都一樣），見 `ask_boxes` 的註解。
    """
