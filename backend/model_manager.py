...
# 用 llama-server 量過各個成分：整張示意圖 1600（不滿一張的比較少）、個人資料每項約 16，
# 配上 filler 的三個夾子（示意圖最多 MAX_PAGES=4 張、勾選題一批 BOX_BATCH=8 題、
# 位置一批 BATCH=16 個），需求是有天花板的——表格再大都不會超過：
#   222（BOXES_SYSTEM）＋ 5400（很厚的履歷）＋ 6400（4 整張圖）＋ 500（8 題）＋ 300（回答）
# 約 12800。太長的表格會縮成剛好 4 整張，圖一定是 6400——以前抓每張 1300，那是兩頁
# 表格「一整張加半張」的平均，推出來的兩個門檻都少了 1200。
# 實測（三份考題接成一份的 4 頁長表格）勾選題那一次：現在的履歷 9234，工作經歷、學歷、
# 證照各多兩三倍的履歷 12148，都還要再加回答。三份考題本身的峰值是 7637。
CTX_FLOOR = 9600        # 低於這個，4 頁的表格根本送不出去——是失敗不是變慢
CTX_COMFORTABLE = 13000  # 到這個以上，履歷很長又碰上 4 頁的表格也還有餘裕
