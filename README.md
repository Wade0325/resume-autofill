...
* **填寫**走結構解析——必須知道每個值要寫回文件的哪一個精確座標，少了座標就寫不回去。
* **匯入**整份交給模型讀——不需要座標，卻要付結構配對錯誤的代價。實測一份 16 欄
  合併儲存格的真實履歷，結構解析會把「聯絡電話」當成姓名的值、「主修科系」當成學校名稱，
  33 列裡約三分之一是垃圾；改用模型直接讀整列後，17 個欄位全部正確，而且從 50 秒縮到 10 秒。

模型讀取的防幻覺手段是**驗證而非祈禱**：回傳的每個值都必須在原文裡逐字找得到，
